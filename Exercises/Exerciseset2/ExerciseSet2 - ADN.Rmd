...
1. 6 principal components are needed to explain at least 50% of the variance
2. The highest value is 0.93
3. We can interpret them as:
Component 1: US Bonds
Component 2: Currencies
Component 3: European stock market indexes
Component 4: North American stock market indexes
Component 5: Euro Bonds
Component 6: Oil prices
Component 7: Industrial Metal prices
Component 8: Food commodities
Component 9: Precious metal prices
Component 10: Japanese stock market indexes
...
corused2 = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results2<-principal(corused2, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors2<-round(Rotated_Results2$loadings,2)
Rotated_Factors2<-as.data.frame(unclass(Rotated_Factors2))
colnames(Rotated_Factors2)<-paste("comp",1:ncol(Rotated_Factors2),sep=" ")

sorted_rows <- sort(Rotated_Factors2[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors2 <- Rotated_Factors2[sorted_rows,]
Rotated_Factors2[abs(Rotated_Factors2) < 0.3]<-NA
```

```{r}
show_data <- Rotated_Factors2 
iprint.df(show_data, scale=TRUE)
```
...
corused3 = cor(crisis_data[,apply(crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results3<-principal(corused3, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors3<-round(Rotated_Results3$loadings,2)
Rotated_Factors3<-as.data.frame(unclass(Rotated_Factors3))
colnames(Rotated_Factors3)<-paste("comp",1:ncol(Rotated_Factors3),sep=" ")

sorted_rows <- sort(Rotated_Factors3[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors3 <- Rotated_Factors3[sorted_rows,]
Rotated_Factors3[abs(Rotated_Factors3) < 0.3]<-NA
```
...
5 From the data, it is possible to see that returns look more correlated during crisis. Also, data are much more concentrated in component 1, and the value of the futures is more heavily correlated on the US Treasury market. On the other hand, during the bull market, the concentration is much less at the top.
...
*
1. The first part of the code is used to normalize the futures table. It shows for all cells 0 values. Next, the MAfreq variable creates a range of numbers which starts at 10 and increments by 20 until 250. Next, a positive or negative sign is created by an iterator that goes through each column and, according to future data. This depicts the predicted trajectory and the "vote sign". $1 is add to each entry in lines 193-198 . Then this is multiplied by the vote value.


2. In order to change the strategy, new variables were created modifying the same code structure. What changed was the levers that are shaping the strategy. For example in the MAfreq range, the moving average is extended and extended the increment. The scrub time period was also changed, moving back to 3 days instead of 2. This modifies the strategy.

```{r echo=TRUE, tidy=TRUE}
signal_used_test = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,250,by=25)
for (iter in 1:length(MAfreq))
  signal_used_test = signal_used_test + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used_test = t(apply(signal_used_test,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
  res
}))
colnames(signal_used_test) <- colnames(futures_data)
# Now create the returns of the strategy for each futures time series
strategy_by_future_test <- scrub(shift(signal_used_test,3)*futures_data) # signal 2 days ago
# finally, this is our futures trend following strategy
trading_strategy_test = apply(strategy_by_future_test,1,sum)
names(trading_strategy_test) <- rownames(futures_data)
```
```{r}
iplot.pnl(trading_strategy_test)
```
```{r}
show_data = round(pnl_matrix(trading_strategy_test),2)
iprint.df(show_data)
```
